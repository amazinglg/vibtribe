...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    ...
    links: [
      ...
      { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=Sora:wght@500;600;700&display=swap" },
    ...
    scripts: [
      {
        // Paint the brand-dark background on the very first frame so iOS
        // PWA / Safari / Android WebView never flash white while the CSS
        // bundle is still parsing. Runs before styles.css loads.
        children:
          "document.documentElement.style.background='#070a1b';document.documentElement.style.colorScheme='dark';",
      },
      {
        // Pre-hydration session hint: the homepage is server-rendered with the
        // public landing page (crawlable HTML). If this browser already holds a
        // Supabase session, mark <html> so CSS hides the landing shell before
        // first paint - signed-in users never see a landing-page flash.
        children:
          "try{for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k&&/^sb-.*-auth-token$/.test(k)&&localStorage.getItem(k)){document.documentElement.classList.add('vt-authed');break;}}}catch(e){}",
      },
      {
        children:
          "(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-K5LZB9ML');",
      },
      {
